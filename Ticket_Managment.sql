...
WHERE EXISTS (
    ...
    JOIN Event e ON b.event_id = e.event_id
    WHERE b.customer_id = c.customer_id
    AND MONTH(b.booking_date) IN (4, 5)  -- Check for bookings in April (4) and May (5)
    GROUP BY c.customer_id
    HAVING COUNT(DISTINCT MONTH(b.booking_date)) = 2  -- Ensure bookings in both April and May
);
